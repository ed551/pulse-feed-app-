...
import { Youtube, RefreshCw, Play, Loader2, Tv, Radio } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import { cn } from '../lib/utils';
import MediaChannelsSection from './MediaChannelsSection';
...
export default function YouTubeSection() {
  ...
  const [activeTab, setActiveTab] = useState<'youtube' | 'channels'>('youtube');
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-[2rem] p-6 shadow-xl border border-gray-100 dark:border-gray-700 relative overflow-hidden group">
      ...
      <div className="relative z-10">
        <div className="flex bg-gray-50 dark:bg-gray-900/50 p-1 rounded-2xl border border-gray-100 dark:border-gray-700 w-full mb-6">
          <button
            onClick={() => setActiveTab('youtube')}
            className={cn(
              "flex-1 flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all",
              activeTab === 'youtube'
          ...
          >
            <Tv className="w-4 h-4" /> Live TV & Radio
          </button>
        </div>
